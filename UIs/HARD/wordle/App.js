        ...
        lettersRow.map(({ char, state }, colIndex) => (
          <div
            className={clsx(
              ...
              state !== LETTER_GUESS_STATE.INDETERMINATE &&
                clsx("grid-cell--final", GUESS_CLASSES[state])
            )}
            style={{
              transitionDelay:
                state !== LETTER_GUESS_STATE.INDETERMINATE
...
function GameResult({ gameState, wordOfTheDay, onResetClick }) {
  ...
  return (
    <div className="result-row">
      <strong>
        <>
          {gameState === GAME_STATE.GUESSED_CORRECTLY && "Congratulations 🎉"}
        </>
        <>
          {gameState === GAME_STATE.NO_MORE_GUESSES && `Word: ${wordOfTheDay}`}
        </>
      </strong>
      <button type="button" className="reset-button" onClick={onResetClick}>
        Reset
      </button>
    </div>
  );
...
function Wordle({ maxAttempts, lettersPerWord }) {
  ...
  const [wordOfTheDay, setWordOfTheDay] = useState(generateRandomWord());
  ...
  const [position, setPosition] = useState(INITIAL_CURSOR_POSITION);
  ...
  useEffect(() => {
    ...
    console.log(`[DEBUG]: Word of the day is: ${wordOfTheDay}`);
  }, [wordOfTheDay]);

  const resetGame = useCallback(() => {
    setWordOfTheDay(generateRandomWord());
    setGridState(getInitialGridState(maxAttempts, lettersPerWord));
  ...
  function checkWord() {
    ...
    nonMatchingIndices.forEach((idx) => {
      ...
      if (letterFreq.has(char) && letterFreq.get(char) > 0) {
        ...
        if (
          LETTER_GUESS_STATE_PRIORITY[LETTER_GUESS_STATE.PRESENT] >
          LETTER_GUESS_STATE_PRIORITY[
            newLetterGuessState[char] ?? LETTER_GUESS_STATE.INDETERMINATE
          ]
        ) {
          newLetterGuessState[char] = LETTER_GUESS_STATE.PRESENT;
        }
      } else {
        newGridState[row][idx].state = LETTER_GUESS_STATE.ABSENT;
        // Only change state if the new state is higher priority
        // than the current state.
        if (
          LETTER_GUESS_STATE_PRIORITY[LETTER_GUESS_STATE.ABSENT] >
          LETTER_GUESS_STATE_PRIORITY[
            newLetterGuessState[char] ?? LETTER_GUESS_STATE.INDETERMINATE
          ]
        ) {
